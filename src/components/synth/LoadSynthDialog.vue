...
function load(checkMissingChannels: boolean = true) {
	...
	if (missingDevices.size > 0) {
		...
		missingMidiDevices.value = Object.fromEntries(
			Array.from(missingDevices.entries()).map(([id, name]) => [id, name]),
		)
	...
	SynthSerializer.load(synth, data, includedCategories.value)
	...
	>
				...
				<div class="flex-stretch">
					<div class="control-item" id="preset-name">
						...
						<el-select
							v-model="synthName"
							@change="selectPreset"
							@clear="enableAllCategories()"
							filterable
							clearable
							default-first-option
						>
							<el-option
								v-for="(synthPreset, name) in synthPresets"
								:key="name"
								:value="name"
							...
							>
								...
								<div class="preset-tags-container">
									<el-tag
										type="primary"
										v-for="category in SynthSerializer.getPresetCategoryNames(synthPreset)"
										>{{ category }}</el-tag
									>
								</div>
							</el-option>
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" :disabled="!isAnyCategorySelected()" @click="load()"
					>Load</el-button
				>
			</span>
		</template>
		...
		>
			<p id="missing-devices-message">
				Some of the provided MIDI devices no longer exist. Assign them to an existing device or
				leave blank to skip channels for that device:
			</p>
