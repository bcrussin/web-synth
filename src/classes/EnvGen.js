...
export default function EnvGen(audioContext, targetParam) {
...
EnvGen.prototype._scheduleSegmentFromTime = function (time, targetValue, timeConst) {
  ...
  for (var i = 0; i < this._scheduledSegments.length; i++) {
    if (
      ...
      (i === this._scheduledSegments.length - 1 || time < this._scheduledSegments[i + 1].beginTime)
    ) {
      activeIdx = i
      break
    }
...
EnvGen.prototype._scheduleSegmentFromValueThreshold = function (
  ...
  var interruptTime =
    Math.abs(
      Math.log(
        (lastSeg.targetValue - valueThreshold) / (lastSeg.targetValue - lastSeg.beginValue),
      ) * lastSeg.timeConst,
    ) + lastSeg.beginTime
