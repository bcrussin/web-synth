  ...
  Object.defineProperty(this, "mode", {
    ...
    set: function (value) {
      if (_this.MODES.indexOf(value) >= 0) {
        ...
        _this.gate(
          false,
          Math.max(this._lastGateTime, audioContext.currentTime)
        );
...
EnvGen.prototype._appendSegment = function (
  beginTime,
  beginValue,
  targetValue,
  timeConst
  ...
  assert(
    beginTime >=
      this._scheduledSegments[this._scheduledSegments.length - 1].beginTime
  ); // sanity check
...
EnvGen.prototype._scheduleSegmentFromTime = function (
  time,
  targetValue,
  timeConst
) {
  ...
  for (var i = 0; i < this._scheduledSegments.length; i++) {
    if (
      time >= this._scheduledSegments[i].beginTime &&
      (i === this._scheduledSegments.length - 1 ||
        time < this._scheduledSegments[i + 1].beginTime)
    ) {
      activeIdx = i;
      break;
    }
  ...
  var interruptValue =
    activeSeg.targetValue +
    (activeSeg.beginValue - activeSeg.targetValue) *
      Math.exp((activeSeg.beginTime - time) / activeSeg.timeConst);
...
EnvGen.prototype._scheduleSegmentFromValueThreshold = function (
  valueThreshold,
  targetValue,
  timeConst
) {
  ...
  var interruptTime =
    Math.abs(
      Math.log(
        (lastSeg.targetValue - valueThreshold) /
          (lastSeg.targetValue - lastSeg.beginValue)
      ) * lastSeg.timeConst
    ) + lastSeg.beginTime;
...
EnvGen.prototype.gate = function (on, time) {
  ...
  if (on) {
    ...
    var attackTargetLevel =
      1 / (1 - Math.exp(-this._attackTime / ATTACK_LINEARITY));
    ...
    if (this._mode === "AD" || this._mode === "ADSR") {
      ...
      // Schedule decay
      this._scheduleSegmentFromValueThreshold(
        ATTACK_LEVEL,
        decayTargetLevel,
        this._decayTime
      );
